...
def frame3d_curvne(file: Path, xg: dict[str, T.Any] = None) -> xarray.Dataset:
    ...
    with h5py.File(file, "r") as f:
        ...
        # trymes = ('J1', 'J1all', 'J2', 'J2all', 'J3', 'J3all', 'Phitop', 'Te', 'Ti', 'ne', 'time', 'v1', 'v2', 'v3')
        trymes = ('J1', 'J2', 'J3',
                  'Phitop',
                  'Te', 'Ti',
                  'ne', 'time',
        ...
        print("HACK IN ~/Research/pygemini/src/gemini3d/hdf5/read.py: Grab more variables in frame3d_curv!")

        for tryme in trymes:
            try:
                dat[tryme] = (("x1", "x2", "x3"), f["/"+tryme][:].transpose(p3))

            except:
                print(f"Couldn't get '{tryme}' from HDF5 file!")

        dat['Phitop'] = (("x2","x3"),f["/"+"Phitop"][:].transpose(p3[1:]))
