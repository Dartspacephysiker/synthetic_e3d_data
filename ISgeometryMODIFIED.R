...
bistaticResolutions.single <- function(refPoint,locTrans,locRec,locxy=F,fwhmTrans=1,fwhmRec=1,fwhmRange=1,fwhmIonSlab=100,x=seq(-300,300,by=10),y=seq(-300,300,by=10),heights=c(100,200,500,1000),infinity=defaultInfinity(),phArrTrans=TRUE,phArrRec=TRUE,mineleTrans=0,mineleRec=0){
  #
  # resolutions at points c(x,y,height), where x is measured along a circle of latitude, and y along a meridian. The distances are  measured from the point refPoint
  #
  # INPUT:
  ...
  res          <- array(dim=c(nx,3))
  kscat        <- array(dim=c(nx,3))
  distTS       <- array(dim=c(nx))
  distRS       <- array(dim=c(nx))
  vecTSh       <- array(dim=c(nx,3))
  gainInt      <- array(dim=c(nx))
  gainIntNoise <- array(dim=c(nx))
  volume       <- array(dim=c(nx))
  volumeNoise  <- array(dim=c(nx))
  beta         <- array(dim=c(nx))
  ...
  for(i in seq(nx)){
    ...
    scatPos        <- planarToSpherical.geographic(x=x[i],y=y[i],refPoint=refPoint)
    xyzScat        <<- sphericalToCartesian(c(scatPos$lat,scatPos$lon,EarthRadius()+heights[i]))
    res[i,]    <- get3Dresolution.cartesian(
      locTrans=xyzT,locRec=xyzR,locScat=xyzScat,fwhmTrans=fwhmTrans,fwhmRec=fwhmRec,
      fwhmRange=fwhmRange,fwhmIonSlab=fwhmIonSlab,infinity=infinity,phArrTrans=phArrTrans,phArrRec=phArrRec,
      mineleTrans=mineleTrans,mineleRec=mineleRec)
    ...
    # the scattering wave vector
    kscatxyz       <- scatterPlaneNormal.cartesian(xyzT,xyzR,xyzScat)
    kscat[i,]  <- rotateHorizontal.vector.cartesian(kscatxyz,pos=xyzScat)
    
    # distances from the transmitter and the receiver to the target
    distTS[i]  <- sqrt(sum(abs(xyzT-xyzScat)**2))
    distRS[i]  <- sqrt(sum(abs(xyzR-xyzScat)**2))
    
    vecTSh[i,] <- rotateHorizontal.vector.cartesian( ((xyzT-xyzScat)/distTS[i]) , pos=xyzScat )
    ...
    if (phArrTrans)
    {
      fwhmBeam <- fwhmTrans*pi/180
      fwhmBeamWidened <- tiltedBeamWidth(fwhmBeam,alphaT)
      intTbeam <- 2 * pi * distTS[i]**2 * fwhm2std(fwhmTrans*pi/180) * fwhm2std(fwhmBeamWidened)
    }
    else
    {
      intTbeam <- 2 * pi * distTS[i]**2*fwhm2std(fwhmTrans*pi/180)**2
    }
    ...
    if (phArrTrans)
    {
      fwhmBeam <- fwhmRec*pi/180
      fwhmBeamWidened <- tiltedBeamWidth(fwhmBeam,alphaR)
      intRbeam <- 2 * pi * distRS[i]**2 * fwhm2std(fwhmRec*pi/180) * fwhm2std(fwhmBeamWidened)
    }
    else
    {
      intRbeam <- 2 * pi * distRS[i]**2*fwhm2std(fwhmRec*pi/180)**2
    }
    
    
    volume[i]       <- (2*pi)**(3/2)*sqrt(det(covarScattVol))
    volumeNoise[i]  <- (2*pi)**(3/2)*sqrt(det(covarNoise))
    
    gainInt[i] <- 1e-3*volume[i]/intTbeam/intRbeam*ifelse(phArrTrans,cos(alphaT),1)
    
    gainIntNoise[i] <- 1e-3*volumeNoise[i]/intTbeam/intRbeam
    
    # angle between scattering wave vector and incident wave vector
    beta[i]    <- vectorAngle.cartesian((xyzScat-xyzT),kscatxyz,degrees=T)
  }
...
vectorVelocityVariance <- function(kscats=list(),angle=list(),vars,ptlist=F){
#
  ...
  if(n!=nv) stop('Lengths of k and variance lists are different.')
    ...
    if (ptlist) {
      ...
      for(i in seq(npts)){
        ...
        for(l in seq(n)){
          A[l,] <- kscats[[l]][i,]
          invsigma[l,l] <- 1/vars[[l]][i]
        }
        kvars[i,] <- tryCatch(diag(solve(t(A)%*%invsigma%*%A)),error=function(e){return(rep(NA,3))})
      }
...
absoluteNoiseLevel <- function(resolutions,intCoefs,Pt=1e6,Ne=1e11,Tnoise=300,tau0,fradar=233e6,RXduty=1,verbose=F,ptlist=F){
#
...
#   mineleRec  minimum allowed elevation for receiver(s) in degrees [0 90]
#
#   ptlist      boolean, TRUE if x,y,h should be treated as point pairs rather than lists of points that, when looped over, form a grid
#  OUTPUT:
#   a list with elements:
#     x                     same as input
#     y                     same as input
...
#
    ...
    for(i in seq(nTrans)){
        for(j in seq(nRec)){
            ...
            # gain integrals etc.
            if (ptlist) {
              cat(sprintf("YESPTLIST i, j: %d, %d\n",i,j))
              noiseLevels[[n]] <- bistaticResolutions.single(refPoint=refPoint,locTrans=locTrans[[i]],locRec=locRec[[j]],locxy=locxy,fwhmTrans=fwhmTrans[i],fwhmRec=fwhmRec[j],fwhmRange=fwhmRange[j],fwhmIonSlab=fwhmIonSlab,x=x,y=y,heights=heights,infinity=infinity,phArrTrans=phArrTrans[i],phArrRec=phArrRec[j],mineleTrans=mineleTrans[i],mineleRec=mineleRec[j])
            } else {
              cat(sprintf("NOPTLIST i, j: %d, %d\n",i,j))
            ...
            # ACF noise level
            noiseLevels[[n]]$noiseLevel <- absoluteNoiseLevel(resolutions=noiseLevels[[n]],intCoefs=intCoefs,
                                                              Pt=Pt[i],Ne=Ne,
                                                              Tnoise=Tnoise[j],fradar=fradar,
                                                              tau0=tau0,RXduty=RXduty[j],
                                                              verbose=verbose,ptlist=ptlist)
        }
    }
    ...
    velvars <- vectorVelocityVariance(kscats,angles,vars,ptlist=ptlist)

    noiseLevel.velocity <- noiseLevels[[1]]$noiseLevel
    if (ptlist) {
      noiseLevel.velocity$noiseLevel<- sqrt(apply(velvars,MARGIN=1,FUN=sum))  
    } else {
